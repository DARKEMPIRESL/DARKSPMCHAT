...
db = Database(DATABASE_URL, "DARKS_PM")     
...
@Client.on_message(filters.incoming & filters.chat(-1001697574951))
async def bchanl(bot, update, broadcast_ids={}): 
    ...
    out = await bot.send_message(-1001722815470,f"Ads Broadcast Started! You will be notified with log file when all the users are notified.")
    start_time = time.time()
    total_users = await db.total_users_count()
    done = 0
    failed = 0
    ...
    if failed == 0:
        await bot.send_message(-1001722815470, f"broadcast completed in `{completed_in}`\n\nTotal users {total_users}.\nTotal done {done}, {success} success and {failed} failed.")
    else:
        await bot.send_document(-1001722815470, 'broadcastlog.txt', caption=f"broadcast completed in `{completed_in}`\n\nTotal users {total_users}.\nTotal done {done}, {success} success and {failed} failed.")
